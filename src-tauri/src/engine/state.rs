...
mod tests {
    ...
    fn safe_string_strips_extended_length_prefix() {
        let p = std::path::Path::new(r"\\?\D:\Program Files\FFDownloader\binaries\aria2.conf");
    ...
    fn safe_string_preserves_unix_path() {
        let p = std::path::Path::new("/usr/local/share/ff-downloader/binaries/aria2.conf");
        let result = path_to_safe_string(p);
        assert_eq!(result, "/usr/local/share/ff-downloader/binaries/aria2.conf");
    }
