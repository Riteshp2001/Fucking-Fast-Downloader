...
///
/// The fix: register a custom `NSProgressIndicator` subclass (`FFDownloaderProgressIndicator`)
...
/// or creates a new `FFDownloaderProgressIndicator` (custom subclass with `drawRect:` override).
///
...
/// Registers the `FFDownloaderProgressIndicator` ObjC class (once) — a custom
...
fn register_progress_indicator_class() -> *const objc2::runtime::AnyClass {
    ...
    INIT.call_once(|| unsafe {
        ...
        let mut decl = ClassBuilder::new(c"FFDownloaderProgressIndicator", superclass)
            .expect("Failed to create FFDownloaderProgressIndicator class");
...
async fn stat_loop(
    ...
    loop {
        ...
        if let Some(rc_state) = app.try_state::<RuntimeConfigState>() {
            ...
            if let Some(tray) = app.tray_by_id("ff-downloader") {
                let next_title =
