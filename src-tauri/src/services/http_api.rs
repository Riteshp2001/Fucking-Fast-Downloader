...
/// Build a `ffdownloader://new?url=X&referer=Y&cookie=Z` deep-link URL.
///
...
fn build_deep_link_url(req: &AddRequest) -> String {
    let mut deep_link = url::Url::parse("ffdownloader://new").expect("static URL must parse");
    {
...
mod tests {
    ...
    fn deep_link_url_encodes_basic_url() {
        let mut deep_link = url::Url::parse("ffdownloader://new").unwrap();
        deep_link
            .query_pairs_mut()
            .append_pair("url", "https://example.com/file.zip");
        assert!(deep_link.to_string().contains("url=https"));
        assert!(deep_link.to_string().starts_with("ffdownloader://new?"));
    }
    ...
    fn deep_link_url_includes_filename() {
        ...
        assert!(result.starts_with("ffdownloader://new?"));
