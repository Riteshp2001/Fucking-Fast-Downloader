...
mod tests {
    ...
    fn filters_supported_external_inputs_from_argv() {
        let args = vec![
            "/Applications/FFDownloader.app".to_string(),
            "--flag".to_string(),
    ...
    fn take_pending_deep_links_preserves_silent_intent() {
        let state = PendingDeepLinkState::new();
        {
            ...
            append_unique_pending(
                &mut inner.queue,
                &["ffdownloader://new?url=https%3A%2F%2Fexample.com%2Ffile.zip".to_string()],
            );
            inner.silent = true;
        }
        ...
        assert_eq!(
            payload.urls,
            vec!["ffdownloader://new?url=https%3A%2F%2Fexample.com%2Ffile.zip".to_string()]
        );
