...
pub(crate) fn read_log_level() -> log::LevelFilter {
    (|| -> Option<log::LevelFilter> {
        let data_dir = dirs::data_dir()?.join("com.ff-downloader.app");
...
fn setup_app(app: &mut tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // native ~8px corner rounding, matching the original look.
    //
...
pub fn run() {
    ...
    if let Some(dir) = dirs::data_dir().map(|d| d.join("com.ff-downloader.app")) {
        db_guard::check(&dir);
    }
            ...
            tauri_plugin_log::Builder::new()
                .targets([
                    tauri_plugin_log::Target::new(tauri_plugin_log::TargetKind::LogDir {
                        file_name: Some("ff-downloader".into()),
                    }),
    ...
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::scrape_fitgirl_page,
            commands::resolve_fuckingfast_link,
            commands::scrape_and_resolve,
