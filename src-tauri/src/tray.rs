...
pub fn setup_tray(app: &AppHandle) -> Result<TrayMenuState, Box<dyn std::error::Error>> {
    ...
    let show_item = MenuItem::with_id(app, "show", "Show FF Downloader", true, None::<&str>)?;
    ...
    let _tray = TrayIconBuilder::with_id("ff-downloader")
        .menu(&menu)
        .show_menu_on_left_click(false)
        .tooltip("FF Downloader")
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Some(tray) = app_handle.tray_by_id("ff-downloader") {
