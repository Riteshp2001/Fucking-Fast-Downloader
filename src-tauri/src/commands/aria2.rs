...
mod tests {
    ...
    fn ed2k_search_temp_paths_stay_inside_search_cache_dir() {
        let root = PathBuf::from("/tmp/ff-downloader-ed2k-search");
    ...
    fn ed2k_search_temp_paths_allow_per_search_temp_dirs() {
        let root = PathBuf::from("/tmp/ff-downloader-ed2k-search-abc123");
        let paths = ed2k_search_temp_paths(&root, "75c1fb5d8979819f").expect("valid gid");

        assert_eq!(
            paths[0],
            PathBuf::from(
                "/tmp/ff-downloader-ed2k-search-abc123/ff-downloader-ed2k-search-75c1fb5d8979819f"
            )
        );
        assert_eq!(
            paths[1],
            PathBuf::from(
                "/tmp/ff-downloader-ed2k-search-abc123/ff-downloader-ed2k-search-75c1fb5d8979819f.aria2"
            )
        );
    }
